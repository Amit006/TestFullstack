...
var addphotos = function (photos, callback) {
    console.log(' photos: addphotos ', photos);

    mongo.licence.findOne({ _id: photos._id}).then( ( resData, errfind) => {
        // console.log(' photos.Order_Number: ', photos._id, ' resData: ', resData, ' errfind: ', errfind);
        ...
        if(resData){
            ...
            console.log(' Licence: ---', photos);
            mongo.licence.insertOne(photos.data, function (err, data) {
...
var getphotos = function (Onum, callback) {
    // if (ObjectId.isValid(photosId) === false) {
    ...
    mongo.licence.findOne({ "Order_Number": Onum.Order_Number }, function (err, result) {
        ...
        if (result === null) {
            var error = new Error("No photos Found. One Requested.");
            error.status = 404;
            callback(error);
            return;
        }
